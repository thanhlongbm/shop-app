...
const CartScreen = () => {
    ...
    const buttonProps = totalPrice >= 0.1 ? {
        color : Color.secondary,
        onPress : () => {dispatch(orderItems({items : itemList , totalPrice : totalPrice}))}
    } : {
        color : "#777",
        onPress : () => {}
    };
    ...
    <View style = {styles.container}>
        <View style = {styles.totalBox}>
            <View style = {styles.textBox}>
                ...
                <Text style = {{...styles.totalText , color : Color.primary}}>${totalPrice.toFixed(2)}</Text>
            </View>
        ...
        <FlatList 
            ...
            renderItem = {({item}) => <CartItem 
                                       ...
                                       amount = {item.amount}
                                       deletable
                                       onDelete = {() => {dispatch(removeFromCart(item.key))}}
                                       />}
        />
    </View> );
}
